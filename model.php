...
class Model{
    ...
    {
        $query = "SELECT ribbit_count, IF(ribbit IS NULL, 'You have no Tweets.', ribbit) as ribbit, followers, following ";
    ...
    public function getPublicRibbits($user,$q){
        if($q === false)
        {
    ...
    public function getPublicProfiles($user, $q){
        ...
        if($userRes->num_rows > 0){
	        ...
	        while($row = $userRes->fetch_assoc()){
	            $i = $row['id'];
	            $query .= "SELECT " . $i . " AS id, followers, IF(ribbit IS NULL, 'This user has no Tweets.', ribbit) ";
